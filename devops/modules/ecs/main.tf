...
======*/
resource "aws_cloudwatch_log_group" "cleverops" {
  name = "cleverops"

  tags {
    Environment = "${var.environment}"
    Application = "cleverops"
  }
}
...
======*/
resource "aws_ecr_repository" "cleverops_app" {
...
// the task definition for the db container
...
resource "aws_alb" "alb_cleverops" {
  name            = "${var.environment}-alb-cleverops"
...
resource "aws_alb_listener" "cleverops" {
  load_balancer_arn = "${aws_alb.alb_cleverops.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_ecs_service" "web" {
  ...
  network_configuration {
    ...
    subnets         = ["${var.subnets_ids}"] # use public_subnet_ids
    //subnets         = ["${var.public_subnet_ids}"]
  }
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_cleverops_web_cpu_utilization_high"
